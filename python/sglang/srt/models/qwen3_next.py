...
if _is_npu:
    ...
    from sgl_kernel_npu.norm.split_qkv_rmsnorm_rope import split_qkvgate_gemma_rmsnorm_rope
...
class Qwen3HybridAttentionDecoderLayer(nn.Module):
    ...
    def forward_prepare_npu(self, positions, hidden_states, forward_batch):
        qkv, _ = self.qkv_proj(hidden_states)
        if (
                not (forward_batch.forward_mode == ForwardMode.TARGET_VERIFY and self.attn.layer_id != 3)
                and not (forward_batch.forward_mode == ForwardMode.DECODE and self.attn.layer_id not in [0, 3])
        ):
