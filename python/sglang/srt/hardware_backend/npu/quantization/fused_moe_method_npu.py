...
def npu_fused_experts_fp8_decode(
    ...
    **kwargs,
    ...
    hidden_states, expanded_row_idx, expert_tokens, _ = torch.ops.npu.npu_moe_init_routing_v2(
        hidden_states,
        topk_ids,
...
class NPUW4A4MxFp4DynamicMoEMethod(_NPUFusedMoEMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        w13_weight_scale = w13_weight_scale.transpose(-1, -2).reshape(
        ...
        w2_weight_scale = w2_weight_scale.transpose(-1, -2).reshape(
...
class NPUW8A8MxFp8DynamicMoEMethod(_NPUFusedMoEMethodBase):
    ...
    def process_weights_after_loading(self, layer: torch.nn.Module) -> None:
        ...
        if hasattr(layer, "dispatcher"):
            # MXFP8 re-quantizes activations inside the kernel path, so the
            # dispatcher must keep expert inputs in bf16 instead of int8.
