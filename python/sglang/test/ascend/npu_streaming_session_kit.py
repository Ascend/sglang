"""Streaming-session test method mixins, Ascend NPU adapted.

NPU port of `sglang.test.kits.streaming_session_kit.py`. Both mixins live
here so NPU session tests import their kits from a single module.

Pair with `StreamingSessionServerBase` (from sglang.test.server_fixtures.streaming_session_fixture)
...
NPU adaptations vs the CUDA kit, all controlled by class attributes so
test classes only declare what differs:
- `test_kv_cache_inheritance`: Ascend inherits KV page-aligned — the
  inherited length floors to a page boundary — so allowed values are
  page-aligned (`kv_inherit_page_size`, default 128 = Ascend default page
  size; set 1 for exact token match).
- `test_kv_cache_inheritance`: the first chunk is repeated
  (`kv_inherit_first_chunk_repeats`, default 20) so turn-1 KV spans
  multiple pages; otherwise the page-aligned expectation would floor to 0
  and the assertion would be vacuous.
- abort-recovery tests: the long generation's budget is
  `abort_max_new_tokens` (default 30000). The CUDA kit's hard-coded 100000
  exceeds Qwen3-8B's 40960-token context (the standard NPU test model) and
  the server rejects it with HTTP 400.
- `AbortLeakReproKitMixin` is backend-neutral (identical to the upstream
  kit; ported for a single NPU import site).
"""
...
class StreamingSessionKitMixin:
    """Streaming-session KV-inheritance + retract/abort-recovery suite (NPU)."""
    ...
    # Ascend's KV inheritance floors to a page boundary, so allowed values
    # are page-aligned. Must mirror the class's --page-size (1 = exact
    # token match, only if the backend inherits token-exact).
    kv_inherit_page_size = 128

    # Repeats of the first chunk so turn-1 KV spans multiple pages; otherwise
    # the page-aligned expectation floors to 0 and the assert is vacuous.
    kv_inherit_first_chunk_repeats = 20

    # capacity_of_str_len for the KV-inheritance session. Must exceed the
    # repeated first chunk's character length (~39 chars per repeat) when
    # kv_inherit_first_chunk_repeats is scaled up for large pages.
    kv_inherit_session_capacity = 1000

    # Budget for the long generation in the abort-recovery tests. Must fit
    # the model's context with prompt + spec-draft margin (Qwen3-8B: 40960;
    # 30000 + 34 + 4 leaves ~10k headroom).
    abort_max_new_tokens = 30000

    def test_kv_cache_inheritance(self, gen_len=12):
        """Each turn's cached_tokens must equal previous turn's prompt+completion
        (modulo kv_inherit_offsets, page-aligned)."""
        chunks = [
            "Let me tell you something about France."
            * self.kv_inherit_first_chunk_repeats,
        ...
        session_id = requests.post(
            ...
            json={
                "capacity_of_str_len": self.kv_inherit_session_capacity,
                "streaming": True,
            },
        ).json()
        rid = None
        ...
        for turn_idx, chunk_ids in enumerate(chunks_ids):
            ...
            if turn_idx == 0:
                ...
                # (via inherit_kv_states, not radix tree matching), floored to a
                # page boundary on Ascend.
                page = self.kv_inherit_page_size
                allowed = {
                    (prev_kv_len + off) // page * page
                    for off in self.kv_inherit_offsets
                }
                self.assertIn(
                    cached,
                    allowed,
...
class AbortLeakReproKitMixin:
    """Abort-heavy chunked-prefill leak repro. Backend-neutral."""
