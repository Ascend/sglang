...
&& unzip -o ./sgl-kernel-npu-${SGLANG_KERNEL_NPU_TAG}-torch2.8.0-py311-cann8.5.0-${DEVICE_TYPE}-$(arch).zip \
&& ${UV_PIP_INSTALL} --force-reinstall ./deep_ep*.whl ./sgl_kernel_npu*.whl \
&& cd "$(python3 -m pip show deep-ep | grep -E '^Location:' | awk '{print $2}')" \
&& rm -f ./deep_ep_cpp*.so \
