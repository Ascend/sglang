...
| T3 | `test_tokenizer_backend_huggingface` | 正常路径 | `--tokenizer-backend huggingface` 启动 server + generate | 200，含 "Paris" | **新增** |
| T4 | `test_tokenizer_backend_fastokens` | 边界 | `--tokenizer-backend fastokens` 启动 server + generate | 200，含 "Paris" | **新增** |

**结论**: 移植 GPU 用例 (T1,T2) + 新增 E2E (T3,T4)，合并为一个文件。
...
| 参数 | 输出文件 | 移植 | 新增 | 说明 |
|---|---|---|---|---|
| `--tokenizer-backend` | `test_npu_tokenizer_backend.py` | 2 | 2 | GPU unit + server E2E |
...
| 新增 NPU E2E | 4 | tokenizer_backend (huggingface + fastokens)、model_config_parser (auto + hf) |
| 无需测试 | 3 | 无效值×3 + 默认值(detokenizer=1 已有覆盖) |
