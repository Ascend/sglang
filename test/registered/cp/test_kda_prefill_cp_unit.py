...
class TestKDAPrefillCP(unittest.TestCase):
    ...
    def test_ascend_extend_uses_local_cp_conv_boundaries(self):
        ...
        causal_conv = Mock(return_value=torch.zeros(2, 3))
        ...
        causal_conv.assert_called_once()
        conv_call = causal_conv.call_args
        torch.testing.assert_close(
            conv_call.kwargs["query_start_loc"], context.local_cu_seqlens
        )
        torch.testing.assert_close(
            conv_call.kwargs["conv_states"], local_conv_states.transpose(1, 2)
        )
