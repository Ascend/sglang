...
def _pyspy_dump(pid):
    ...
    try:
        ...
        if result.returncode != 0:
            print(
                f"py-spy dump (pid={pid}) exited with code {result.returncode}"
            )
        return output
...
class TestNpuMmWorkerNumAndMmmu(CustomTestCase):
    ...
    def test_image_benchmark_and_thread_count(self):
        ...
        def _monitor():
            ...
            for attempt in range(40):
                ...
                candidate_pids = _find_sglang_server_pids(
    ...
    def test_mmmu_accuracy(self):
        ...
        self.assertGreaterEqual(
            ...
            f"MMMU accuracy ({score}) below threshold "
            f"({MMMU_ACCURACY_THRESHOLD})",
        )
