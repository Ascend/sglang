...
# ---------------------------------------------------------------------------
# Validation tests: parameter range checking (no server launch needed)
# ---------------------------------------------------------------------------

class TestSwaFullTokensRatioValidation(CustomTestCase):
    """Testcase: Verify --swa-full-tokens-ratio boundary validation.

    [Test Category] Parameter
    [Test Target] --swa-full-tokens-ratio
    [Scenario] S3: out-of-range validation
    """

    def _parse(self, extra_args):
        """Parse ServerArgs with given extra arguments.

        Uses model_path="dummy" because the validation for
        swa_full_tokens_ratio is deep in _handle_cache_compatibility,
        which is only reached after model loading succeeds. We test
        the validation condition directly instead of expecting ValueError
        from __post_init__.
        """
    ...
    def test_out_of_range_above(self):
        """Value > 1.0 is stored but fails validation condition."""
        sa = self._parse(["--swa-full-tokens-ratio", "2.0"])
        self.assertEqual(sa.swa_full_tokens_ratio, 2.0)
        self.assertFalse(0 < sa.swa_full_tokens_ratio <= 1.0)

    def test_out_of_range_below(self):
        """Value 0 is stored but fails validation condition."""
        sa = self._parse(["--swa-full-tokens-ratio", "0"])
        self.assertEqual(sa.swa_full_tokens_ratio, 0.0)
        self.assertFalse(0 < sa.swa_full_tokens_ratio <= 1.0)

    def test_out_of_range_negative(self):
        """Negative value is stored but fails validation condition."""
        sa = self._parse(["--swa-full-tokens-ratio", "-0.5"])
        self.assertEqual(sa.swa_full_tokens_ratio, -0.5)
        self.assertFalse(0 < sa.swa_full_tokens_ratio <= 1.0)
