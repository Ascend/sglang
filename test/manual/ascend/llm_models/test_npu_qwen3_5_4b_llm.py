...
class TestQwen3_5_4BGraphWithMTP(GSM8KAscendMixin, CustomTestCase):
    """Testcase: Verify the inference accuracy of Qwen3.5-4B on GSM8K
    with cuda graph and NEXTN speculative decoding.

    [Test Category] Model
    [Test Target] Qwen/Qwen3.5-4B
    [Test Config] Prefill+Decode, cuda graph enabled, NEXTN speculative decoding
    """

    model = QWEN3_5_4B_WEIGHTS_PATH
    accuracy = 0.70
    other_args = [
        "--trust-remote-code",
        "--mem-fraction-static",
