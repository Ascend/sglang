...
<table style={{width: "100%", borderCollapse: "collapse", tableLayout: "fixed"}}>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.02)"}}>W4A8</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}><a href="#glm-5-1-16k-1k-23ms-on-a3-32-cards-disaggregation-mode">Optimal Configuration</a></td>Expand commentComment on line R905Resolved
    </tr>
    <tr>
      <td style={{padding: "9px 12px", fontWeight: 500, backgroundColor: "rgba(255,255,255,0.02)"}}>GLM-5.1</td>
      <td style={{padding: "9px 12px", backgroundColor: "rgba(255,255,255,0.05)"}}>Atlas 800I A3</td>
...
Expand commentComment on line R6128Resolved
for i in "${!P_IP[@]}";
do
    if [[ "$LOCAL_HOST1" == "${P_IP[$i]}" || "$LOCAL_HOST2" == "${P_IP[$i]}" ]];
    then
        ...
        --quantization modelslim \Expand commentComment on line R6150Resolved
        --speculative-draft-model-quantization unquant \Expand commentComment on line R6151Resolved
        --moe-a2a-backend deepep --deepep-mode auto \
        --load-balance-method round_robin \
        --speculative-algorithm NEXTN --speculative-num-steps 3 --speculative-eagle-topk 1 --speculative-num-draft-tokens 4
        NODE_RANK=$i
...
do
    ...
    then
        ...
        export SGLANG_ZBAL_LOCAL_MEM_SIZE=61184Expand commentComment on line R6241Resolved
        export SGLANG_ENABLE_TP_MEMORY_INBALANCE_CHECK=0
        export PYTORCH_NPU_ALLOC_CONF=expandable_segments:True
        ...
        --speculative-draft-model-quantization unquant \Expand commentComment on line R6259Resolved
        --speculative-algorithm NEXTN --speculative-num-steps 1 --speculative-eagle-topk 1 --speculative-num-draft-tokens 2
        NODE_RANK=$i
        break
    fi
...
do
    ...
    then
        ...
        --speculative-draft-model-quantization unquant \Expand commentComment on line R6289Resolved
        --speculative-algorithm NEXTN --speculative-num-steps 3 --speculative-eagle-topk 1 --speculative-num-draft-tokens 4
        NODE_RANK=$i
        break
    fi
...
do
    ...
    then
        ...
        --disable-shared-experts-fusion --disable-cuda-graph --dtype bfloat16 \
        --speculative-draft-model-quantization unquant \Expand commentComment on line R6394Resolved
        --enable-nsa-prefill-context-parallel \
        --nsa-prefill-cp-mode in-seq-split \
        --attn-cp-size 4 \
        --enable-dp-lm-head --moe-dense-tp 1 \
...
do
    ...
    then
        ...
        --tokenizer-worker-num 4 --disable-shared-experts-fusion --dtype bfloat16  --load-balance-method round_robin \
        --speculative-draft-model-quantization unquant \Expand commentComment on line R6430Resolved
        --speculative-algorithm NEXTN --speculative-num-steps 3 --speculative-eagle-topk 1 --speculative-num-draft-tokens 4 \
...
```
Expand commentComment on line R6438Resolved
```shell Command
python -m sglang_router.launch_router \
    --pd-disaggregation \
    --policy round_robin \
...
do
    ...
    then
        ...
        --served-model-name glm-5 --chunked-prefill-size 16384 --max-prefill-tokens 180000 --moe-a2a-backend deepep --deepep-mode normal \
        --disable-shared-experts-fusion --disable-cuda-graph --dtype bfloat16 \
        --speculative-draft-model-quantization unquant \Expand commentComment on line R6540Resolved
        --enable-nsa-prefill-context-parallel \
        --nsa-prefill-cp-mode in-seq-split \
        --attn-cp-size 4 \
        --enable-dp-lm-head --moe-dense-tp 1 \
...
do
    ...
    then
        ...
        --served-model-name glm-5 --chunked-prefill-size 16384 --max-prefill-tokens 180000 --moe-a2a-backend deepep --deepep-mode normal \
        --disable-shared-experts-fusion --disable-cuda-graph --dtype bfloat16 \
        --speculative-draft-model-quantization unquant \Expand commentComment on line R6579Resolved
        --enable-nsa-prefill-context-parallel \
        --nsa-prefill-cp-mode in-seq-split \
        --attn-cp-size 4 \
        --enable-dp-lm-head --moe-dense-tp 1 \
...
do
    ...
    then
        ...
        --speculative-draft-model-quantization unquant \Expand commentComment on line R6614Resolved
        --speculative-algorithm NEXTN --speculative-num-steps 3 --speculative-eagle-topk 1 --speculative-num-draft-tokens 4 \
...
```
Expand commentComment on line R6622Resolved
```shell Command
python -m sglang_router.launch_router \
    --pd-disaggregation \
    --policy round_robin \
