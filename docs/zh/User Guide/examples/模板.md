...
## 2 特性矩阵（Feature Matrix）

介绍该模型支持的特性，包括支持的硬件、量化方式、数据并行、长序列特性等。

**资料写作要求：**  

- 采用表格的形式，呈现模型和特性的支持情况。
- 或者引用参考，需添加超链接。
...
| ------ | ---------- | ------ | ------ | ---------- | ------ | ------------ | -------------- | ------ | ---------- | ---------- | ---------- | ------------ | ---------- | ---------- | ------------------- |----------- | ----------- | ------------- | ------------- | ---------- |
| DeepSeek V3/3.1 | ✅ | | ✅ | Atlas 800I A2:<br>最低卡数要求为xx | ✅ | ✅ | ✅ | | ✅ | | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | 240k | | [链接](../../tutorials/DeepSeek-V3.1.md) |
| DeepSeek V3.2 | ✅ | | ✅ | Atlas 800I A2:<br>最低卡数要求为xx | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | 160k | ✅ | [链接](../../tutorials/DeepSeek-V3.2.md)|
| DeepSeek R1 | ✅ | | ✅ | Atlas 800I A2:<br>最低卡数要求为xx | ✅ | ✅ | ✅ | | ✅ | | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | 128k | | [链接](../../tutorials/DeepSeek-R1.md) |
| Qwen3 | ✅ | | ✅ | Atlas 800I A2:<br>最低卡数要求为xx | ✅ | ✅ | ✅ | | | ✅ | ✅ | | | ✅ | | ✅ | ✅ | 128k | ✅ | [链接](../../tutorials/Qwen3-Dense.md) |
...
请参考[功能指南](../user_guide/feature_guide/index.md)获取功能配置信息。

## 3 环境准备（Environment Preparation）
...
| 模型版本 | 硬件要求 | 下载链接 |
| ---------- | ---------- | ---------- |
| DeepSeek-V3.2-Exp (BF16) | 2×Atlas 800 A3 (64G×16)<br>4×Atlas 800 A2 (64G×8) | [模型权重](https://modelers.cn/models/Modelers_Park/DeepSeek-V3.2-Exp-BF16) |
| DeepSeek-V3.2-Exp-w8a8 (量化版) | 1×Atlas 800 A3 (64G×16)<br>2×Atlas 800 A2 (64G×8) | [模型权重](https://modelers.cn/models/Modelers_Park/DeepSeek-V3.2-Exp-w8a8) |
| DeepSeek-V3.2-w8a8 (量化版) | 1×Atlas 800 A3 (64G×16)<br>2×Atlas 800 A2 (64G×8) | [模型权重](https://www.modelscope.cn/models/vllm-ascend/DeepSeek-V3.2-W8A8/) |
...
- 提供具体的步骤与启动命令，涵盖单节点与多节点配置。
- 参数需要给出解释，包括含义、取值范围、单位等。
- 需要开启的基础环境变量和通信环境变量，参数要给出解释，包括含义、取值范围、单位。
...
**资料写作要求：** 指导用户如何在服务启动后，通过简单接口测试模型的基本功能是否正常。
...
| 优化技术 | 技术原理 | 启用方式 | 适用场景 | 注意事项 |
| --------- | --------- | --------- | --------- | --------- |
| FlashComm_v1 | 将传统Allreduce分解为Reduce-Scatter和All-Gather，减少RMSNorm计算维度 | `export VLLM_ASCEND_ENABLE_FLASHCOMM1=1` | 大并发、张量并行(TP)场景 | 阈值保护：仅当实际token数超过阈值时生效，避免小并发场景性能倒退|
| Matmul-ReduceScatter融合 | 将矩阵乘法与Reduce-Scatter操作融合，实现流水线并行处理 | 启用FlashComm_v1后自动开启 | 大型分布式环境 | 同FlashComm_v1，有阈值保护 |
| 权重预取 | 利用向量计算时间，提前将MLP权重加载到L2 Cache | `export VLLM_ASCEND_ENABLE_PREFETCH_MLP=1` | MLP密集型场景（Dense模型） | 需配合预取缓冲区大小调整 |
| 异步调度 | 非阻塞任务调度，提升并发处理能力 | `--async-scheduling` | 大规模模型、高并发场景 | 与FullGraph优化协同使用 |
...
**资料写作要求：** 提供一些常见问题的解决措施，包括但不限于问题现象描述、原因分析、解决措施。
