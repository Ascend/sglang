...
RUN python3 -m pip install --no-cache-dir pybind11 setuptools wheel && \
    ...
    ./memfabric_hybrid-*_linux-$(arch).run --install && \
...
# Install SGLang
RUN git clone https://github.com/sgl-project/sglang /sgl-workspace/sglang && \
...
RUN mkdir cann-custom-ops && \
    ...
    wget -O cann-ops-transformer-custom_linux-$(arch).run "https://sglang-ascend.obs.cn-east-3.myhuaweicloud.com:443/dsv41/cann-ops-transformer-custom_linux-aarch64.run?AccessKeyId=HPUAXT4YM0U8JNTERLST&Expires=1789680150&Signature=nIu2UpZryzkP4VVHxl6sWWHiTq8%3D" && \
