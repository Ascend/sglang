...
RUN mkdir cann-custom-ops && \
    cd cann-custom-ops && \
    mkdir -p /usr/local/Ascend/cann-${CANN_VERSION}/runtime/lib64/stub && \
    ln -sf /usr/local/Ascend/cann-${CANN_VERSION}/$(arch)-linux/devlib/device/libascend_hal.so /usr/local/Ascend/cann-${CANN_VERSION}/runtime/lib64/stub/libascend_hal.so && \
    export LD_LIBRARY_PATH=/usr/local/Ascend/cann-${CANN_VERSION}/runtime/lib64/stub:${LD_LIBRARY_PATH} && \
