...
ARG SGLANG_TAG=support_longcatpro_for_npu
ARG ASCEND_CANN_PATH=/usr/local/Ascend/ascend-toolkit
ARG SGLANG_KERNEL_NPU_TAG=0630
...
# Install SGLang
RUN git clone https://github.com/LinyuanLi0046/sglang.git --branch $SGLANG_TAG /sgl-workspace/sglang && \
...
RUN ${PIP_INSTALL} wheel==0.45.1 pybind11 pyyaml decorator scipy attrs psutil \
    ...
    && cd sgl-kernel-npu \
    && wget https://github.com/LinyuanLi0046/sgl-kernel-npu/releases/download/${SGLANG_KERNEL_NPU_TAG}/sgl-kernel-npu-${SGLANG_KERNEL_NPU_TAG}-torch2.10.0-py311-cann${CANN_VERSION}-${DEVICE_TYPE}-$(arch).zip \
